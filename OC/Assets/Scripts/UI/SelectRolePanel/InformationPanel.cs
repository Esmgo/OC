...
public class InformationPanel : MonoBehaviour
{
    ...
    public void ShowRoleInfo(CharacterConfiguration config)
    {
        ...
        if (!isInited)
        {
            CreatWeaponIcon(config);
            isInited = true;
        }
        RefreshWeaponIcon(config);
    }

    public void ShowWeaponInfo(WeaponConfiguration weaponConfig)
    {
        //ShowInfo("攻击间隔", weaponConfig.attackRange, 0, Color.white, weaponInformationContent, weaponInfoBars, false, 2);
    }
    ...
    public void CreatWeaponIcon(CharacterConfiguration roleConfig)
    {
        for (int i = 0; i < 5; i++)
        {
    ...
    public void RefreshWeaponIcon(CharacterConfiguration roleConfig)
    {
        for (int i = 0; i < 5; i++)
        {
